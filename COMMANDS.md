...
| `/collect-digest [--since=어제] [--cap=20]` | 추적 채널(`digests/<주제>/<채널>/`)의 신규 영상을 **무인** 감지→digest→commit·push (사이트 자동 갱신) | 커맨드 |
| `/digest <URL...>` 또는 `/digest --channel=@handle [--all]` | 영상(단건·다중·채널전체)을 digest(시간순 전문 + 주제별 상세요약)로 `digests/<주제>/<채널>/`에 저장. `digest-worker`로 격리 병렬 | 커맨드 |

## 한눈에

- **한 편** 정리 → URL 주고 "정리해줘"
- **여러 편** 한꺼번에 → `/batch-notes`
- **채널 신규 digest 자동** → `/collect-digest` (매일 로컬 루틴 → push → 사이트 갱신)
