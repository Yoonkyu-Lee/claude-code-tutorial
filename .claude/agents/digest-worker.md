...
5. **설명란 반영**(경로가 주어졌을 때만): Read로 읽어 **출처 정보만** 추린다.
   - 살릴 것: 인용 출처 목록(저자·제목·매체·연도), 기사 URL, 챕터 타임스탬프, 협찬·이해관계 고지.
   - 버릴 것: BGM 크레딧, 이메일·SNS 링크, 자사 서비스 홍보, 구독 유도.
   - **본문(paraphrase)에 섞지 않는다.** 설명란은 화자 발언이 아니다. 파일 끝에 `## 설명란 출처 (영상 더보기란)` 섹션을 따로 만들어 원문 그대로 옮기고, 화자가 말한 내용이 아님을 한 줄로 명시한다.
