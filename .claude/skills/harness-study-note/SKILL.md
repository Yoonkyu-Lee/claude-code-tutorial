...
### Step 0: 주제 + 채널 폴더 결정
URL의 채널을 식별해 저장 폴더 `notes/<주제>/<youtube-handle>/`를 정한다 (주제 → 채널 → 노트).

- 호출 측이 `주제: <topic>` + `채널: <handle>`(또는 `채널 폴더: notes/<topic>/<handle>/`)를 같이 줬으면 그대로 사용.
- **주제 결정**: 채널이 이미 어느 주제 폴더(`notes/*/[handle]`) 아래 존재하면 그 주제를 그대로 쓴다(채널=주제 고정). 없으면(새 채널) 사용자에게 주제+채널을 확인한다. 일괄 실행 시엔 메인이 결정해 넘긴다.
- 채널 핸들이 안 왔으면 다음 순으로 추출:
...
- 결정된 폴더가 없으면 만든다. 단 **새 채널/새 주제 폴더 생성**은 단독 실행 시 사용자에게 한 번 확인(주제가 새것이면 `notes/<주제>/INDEX.md`도 함께 만들고 루트 주제 카탈로그에 행 추가 제안). 일괄 실행 시엔 호출 측이 이미 결정한 폴더를 받는다.
- 결정된 폴더 경로를 이후 단계가 사용한다. 변수명: `<NOTES_DIR>` = `notes/<주제>/<handle>/`.
...
채널 폴더의 `<NOTES_DIR>/INDEX.md`(= `notes/<주제>/<채널>/INDEX.md`)는 그 채널 study-note의 색인이다. 한 노트가 새로 생기면 채널 색인에 항목 하나가 들어가야 한다. (상위 카탈로그는 노트 단위로 갱신하지 않는다: `notes/<주제>/INDEX.md`=채널 카탈로그는 새 채널 진입 시, 루트 `index.md`=주제 카탈로그는 새 주제 진입 시 또는 노트 수 표기 갱신 시에만 손댄다.)
...
- **단독 실행**(사용자가 직접 호출 또는 `/study-note`): worker가 직접 `<NOTES_DIR>/INDEX.md`를 편집해 항목을 게시일 순서에 끼워 넣는다. 같은 영상 ID의 기존 항목이 있으면 새 항목으로 덮어쓴다. 그 채널의 첫 노트라 채널 INDEX가 없으면 새로 만든다 + `notes/<주제>/INDEX.md`(채널 카탈로그)에 행 추가; 주제 자체가 새것이면 `notes/<주제>/INDEX.md` 생성 + 루트 주제 카탈로그에 행 추가도 함께 제안.
...
`주제:`/`채널:` 줄은 메인이 어느 채널 INDEX(`notes/<주제>/<채널>/INDEX.md`)에 넣을지 라우팅에 쓰고, 본문에는 옮기지 않는다. 이 블록을 그대로 메인이 그 채널 INDEX에 붙일 수 있게 한다.
