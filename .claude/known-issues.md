...
## 9. 채널 열거에서 일부 영상이 조용히 빠짐 (대부분 정상)

- **문제**: `channel-videos.sh`가 `listed 46/50`처럼 요청보다 적게 반환. 사유를 안 알려줘서 "실패"인지 "원래 못 받는 것"인지 구분이 안 됐다.
- **원인**: flat-playlist는 채널의 **모든** 영상 ID를 주지만, 개별 메타 조회는 접근 제한 영상에서 실패한다. @softdragon 50편 중 4편이 그랬고 **전부 멤버십 전용**이었다(공교롭게도 4편 모두 CEO 인터뷰 특집 — 이 채널은 롱폼 인터뷰를 멤버십 뒤에 둔다).
- **채택된 해결**: 스크립트가 영상별 stderr의 `ERROR:` 줄을 분류해 `<out>.skipped`(`id<TAB>사유<TAB>원문`)에 남기고, stderr에 사유별 집계를 찍는다. 사유 분류:
  - **정상적인 접근 제한** — `members-only` / `private` / `geo-blocked` / `age-restricted` / `removed` / `not-yet-public` / `unavailable`. 자막이 없으니 digest 대상이 아니다. 에이전트는 **후보고 한 줄만** 하고 진행한다. 재시도·쿠키·우회 금지.
  - **조사 대상** — `other` / `no-error-output`. 이때만 원문을 사용자에게 보여준다.
- **함정**: 실패하면 yt-dlp가 `--print-to-file` 대상 파일을 **아예 만들지 않는다**. 파트 파일 glob으로 순회하면 실패 건이 통째로 안 보여서 분류가 조용히 0건이 된다. 항상 생기는 `.id` 사이드카를 기준으로 순회할 것.
- **날짜**: 2026-07-18 확정
