...
- URL ↔ {게시일, 주제, 채널 슬러그} 매핑 표를 만든다. **주제**는 채널이 이미 있는 폴더(`notes/*/[handle]`)면 그 주제를 재사용, 없으면 새 채널로 취급.
- **새 채널/주제 발견 시 사용자 확인**: 입력 URL 중 `notes/<주제>/<핸들>/` 폴더가 아직 없는 채널이 있으면, 진행 전에 그 채널 목록과 **배치할 주제**를 사용자에게 보여주고 "이 채널들을 이 주제로 진행할까요? (`notes/<주제>/<핸들>/` 폴더를 새로 만듭니다)" 확인을 받는다.
...
- 각 worker에게 **URL + 게시일(확정) + 주제 + 채널 슬러그 + 권장 파일명 + 자막파일(clean text 절대경로, 있으면)**을 전달한다. worker에게 "자막파일이 있으면 그걸 자막으로 쓰고, 없거나 비면 MCP(get_timed_transcript→get_transcript)로 폴백하라. 게시일·채널은 그대로 쓰고 yt-dlp/WebSearch/r.jina.ai를 게시일 용도로 호출하지 말라"고 지시한다.
- 저장 경로는 `notes/<주제>/<채널>/<파일명>`. worker는 그 폴더에만 쓴다.
...
2. **중복 점검**: 채널별로, 추출한 블록의 파일명(또는 본문 안의 영상 ID)이 해당 채널 `notes/<주제>/<handle>/INDEX.md`에 이미 있으면 그 항목을 새 블록으로 교체한다. 라우팅 키는 블록의 `주제:`+`채널:`. 같은 영상 ID로 새 노트를 만들어 덮어쓴 경우에 발생.

3. **삽입 위치**: 각 블록은 `## YYYY-MM-DD —`로 시작한다. 게시일 오름차순(오래된 것이 위)을 유지하도록 그 채널 INDEX의 적절한 위치에 끼워 넣는다.

4. **새 채널/주제 처리**: 그 채널의 `notes/<주제>/<handle>/INDEX.md`가 없으면 새로 만들고(헤더 한 줄 + 블록), `notes/<주제>/INDEX.md`(채널 카탈로그)에 새 행 추가. 주제 자체가 새것이면 `notes/<주제>/INDEX.md`를 만들고 루트 `index.md` 주제 카탈로그에 새 행 추가.

5. **카탈로그 카운터 갱신(선택)**: 노트 수·기간이 바뀌면 `notes/<주제>/INDEX.md`(채널 행)와 루트 `index.md`(주제 행)의 카운트·기간을 갱신한다.
...
- 사용자 승인 없이 Step 2 진입 금지 — 잘못 붙여진 URL이 많은 비용을 일으킬 수 있음. (무인 자동화는 notes가 아니라 digest 쪽 `/collect-digest`가 담당하며, 그쪽은 승인 대신 구조적 4중 상한으로 비용을 통제한다.)
