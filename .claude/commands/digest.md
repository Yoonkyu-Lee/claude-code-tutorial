...
- **채널마다 형태가 다르다**: 번호 매긴 「내용 출처」 학술식 인용(softdragon), 기사 URL 직링크(alview), 챕터 타임스탬프+자사 링크(ColorScale), 산문 요약(unrealtech).
- **홍보·제휴 링크도 버리지 않는다.** 채널이 제공한 정보이므로 `## 영상 더보기란` 섹션에 그대로 기록한다. 다만 그 링크의 내용을 다루거나 평가하지는 않는다.
